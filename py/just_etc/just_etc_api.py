...
from . import ETC_py_optimized as ETC_py
...
def load_template(filepath):
    """
    Load a non-stellar galaxy spectral template from a FITS binary or ASCII table.

    The bundled library contains an empty primary HDU followed by a binary
    or ASCII table with WAVELENGTH and FLUX columns. Wavelengths are expected
    in Angstroms and flux density in erg/s/cm²/Å (FLAM).
    ...
    fpath = template_path(filepath)
    if not fpath.exists():
        raise FileNotFoundError(f"Template not found: {fpath}")

    with fits.open(fpath) as hdul:
        # Accept binary tables and the supplied infrared ASCII-table templates.
        tbl = None
        for hdu in hdul:
            if isinstance(hdu, (fits.BinTableHDU, fits.TableHDU)):
                ...
                break
        if tbl is None:
            raise ValueError(f"No FITS table HDU found in {fpath}")
...
class JUSTExposureTimeCalculator:
    ...
    >>> flux, scale = normalize_to_mag(wave, flux, target_mag=20.0, band='r')
    ...
    def __init__(self, spec_file=None, calc_mode='accurate'):
        ...
        spec_file : str or Path, optional
            ...
            Defaults to the bundled data/spec.dat.
        calc_mode : str
    ...
    def solve_exposure_time(self, wave_aa, flux_flam, target_snr,
        ...
        3. Iterate with the Illinois secant/bisection hybrid until
           ``abs(SNR - target_snr) / target_snr < rtol``  or max_iter reached.
